          ...
          <View style={styles.grid}>
            {items.map((item) => (
              <View key={item.id} style={styles.item}>
                ...
                <Text style={[styles.itemMeta, { color: colors.brand }]}>{item.provider} · {item.price}</Text>
                <Text style={[styles.itemDescription, { color: colors.mutedText }]}>{item.description}</Text>
                <View style={styles.tags}>
                  {item.tags.slice(0, 3).map((tag) => (
                    <Text key={tag} style={[styles.tag, { color: colors.brand, backgroundColor: 'rgba(12, 143, 123, 0.10)' }]}>{tag}</Text>
                  ))}
                </View>
                <Link href={{ pathname: '/services/[id]', params: { id: item.id } } as any} asChild>
